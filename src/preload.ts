...
contextBridge.exposeInMainWorld(
  'api', {
    ...
    app: {
      ...
      listFonts: (): Promise<string[]> => { return ipcRenderer.invoke(IPC.APP.FONTS_LIST) },
    ...
    file: {
      ...
      readIcon: (filepath: string): Promise<FileContents> => { return ipcRenderer.invoke(IPC.FILE.READ_ICON, filepath) },
      ...
      getAppInfo: (filepath: string): Promise<ExternalApp> => { return ipcRenderer.invoke(IPC.FILE.GET_APP_INFO, filepath) },
      save: (opts: FileSaveParams): string => { return ipcRenderer.sendSync(IPC.FILE.SAVE_FILE, JSON.stringify(opts)) },
      download: (opts: FileDownloadParams): Promise<string> => { return ipcRenderer.invoke(IPC.FILE.DOWNLOAD, JSON.stringify(opts)) },
    ...
    clipboard: {
      ...
      writeText: (text: string): Promise<boolean> => { return ipcRenderer.invoke(IPC.CLIPBOARD.WRITE_TEXT, text) },
    ...
    history: {
      ...
      loadMetadata: (workspaceId: string): History => { return JSON.parse(ipcRenderer.sendSync(IPC.HISTORY.LOAD_METADATA, workspaceId)) },
      loadChatMessages: (workspaceId: string, chatId: string): any[] => { return JSON.parse(ipcRenderer.sendSync(IPC.HISTORY.LOAD_CHAT_MESSAGES, JSON.stringify({ workspaceId, chatId }))) },
    ...
    automation: {
      ...
      replace: (text: string, sourceApp: Application): Promise<boolean> => { return ipcRenderer.invoke(IPC.AUTOMATION.REPLACE, { text, sourceApp }) },
      insert: (text: string, sourceApp: Application): Promise<boolean> => { return ipcRenderer.invoke(IPC.AUTOMATION.INSERT, { text, sourceApp }) },
    },
    permissions: {
    ...
    docrepo: {
      ...
      connect(baseId: string): Promise<boolean> { return ipcRenderer.invoke(IPC.DOCREPO.CONNECT, baseId) },
      disconnect(): Promise<boolean> { return ipcRenderer.invoke(IPC.DOCREPO.DISCONNECT) },
      create(workspaceId: string, title: string, embeddingEngine: string, embeddingModel: string): Promise<string> { return ipcRenderer.invoke(IPC.DOCREPO.CREATE, { workspaceId, title, embeddingEngine, embeddingModel }) },
      update(baseId: string, title: string, description?: string): Promise<void> { return ipcRenderer.invoke(IPC.DOCREPO.UPDATE, { baseId, title, description }) },
      delete(baseId: string): Promise<void> { return ipcRenderer.invoke(IPC.DOCREPO.DELETE, baseId) },
    ...
    computer: {
      isAvailable: (): Promise<boolean> => { return ipcRenderer.invoke(IPC.COMPUTER.IS_AVAILABLE) },
      ...
      takeScreenshot: (): Promise<string> => { return ipcRenderer.invoke(IPC.COMPUTER.GET_SCREENSHOT) },
      executeAction: (action: ComputerAction): Promise<anyDict> => { return ipcRenderer.invoke(IPC.COMPUTER.EXECUTE_ACTION, action) },
    ...
    memory: {
      ...
      isNotEmpty: (): Promise<boolean> => { return ipcRenderer.invoke(IPC.MEMORY.HAS_FACTS) },
      facts: (): Promise<any[]> => { return ipcRenderer.invoke(IPC.MEMORY.FACTS) },
      store: (content: string): Promise<boolean> => { return ipcRenderer.invoke(IPC.MEMORY.STORE, content) },
      retrieve: (query: string): Promise<string[]> => { return ipcRenderer.invoke(IPC.MEMORY.RETRIEVE, query) },
      delete: (uuid: string): Promise<void> => { return ipcRenderer.invoke(IPC.MEMORY.DELETE, uuid) },
    },
    codeExecution: {
    ...
    backup: {
      export: (): Promise<boolean> => { return ipcRenderer.invoke(IPC.BACKUP.EXPORT) },
      import: (): Promise<boolean> => { return ipcRenderer.invoke(IPC.BACKUP.IMPORT) },
    },
    import: {
      markdown: (): Promise<any> => { return ipcRenderer.invoke(IPC.IMPORT.MARKDOWN) },
      openai: (workspaceId: string): Promise<boolean> => { return ipcRenderer.invoke(IPC.IMPORT.OPENAI, workspaceId) },
    },
    ollama: {
